...
// Restyled to the portfolio's warm palette (cream / warm-grey) instead of the
// original black/white glass look. Uses the existing .glass utility tokens.
const ARROW_CLASSES =
  "relative flex items-center justify-center rounded-full border border-warm-200 dark:border-warm-800 bg-cream/70 dark:bg-warm-900/70 backdrop-blur-md text-warm-600 dark:text-warm-400 cursor-pointer shrink-0 z-30 outline-none hover:border-warm-400 dark:hover:border-warm-500 hover:text-warm-900 dark:hover:text-warm-100 transition-colors duration-300 shadow-sm";
  ...
  return (
    <section className="flex flex-col items-center w-full py-4 lg:py-8 px-4 md:px-8 relative z-20">
      <div className="flex items-center justify-center w-full max-w-[90rem]">
        {/* Fixed card dimensions: tall portrait ratio suits the fan layout, and
            object-cover crops the mixed landscape/portrait source images cleanly. */}
          ...
          {cards.map((card, index) => {
            const image = (
              <div className="relative w-[14rem] h-[20rem] md:w-[16rem] md:h-[22rem] overflow-hidden rounded-2xl shadow-xl border border-warm-200/60 dark:border-warm-800/60 mb-12">
                ...
                />
                {card.caption && (
                  <div className="absolute inset-x-0 bottom-0 z-20 px-4 py-4 bg-gradient-to-t from-black/70 via-black/35 to-transparent">
                    <p className="font-display font-semibold text-lg text-white tracking-wide text-center">
                      {card.caption}
                    </p>
                  </div>
                )}
      ...
      {needsPagination && (
        <div className="flex items-center justify-center gap-4 mt-4 md:mt-6 z-30">
          ...
          <div className="flex items-center gap-2">
            {cards.map((_, i) => (
              <span key={i} className={`w-2 h-2 rounded-full transition-all duration-300 ${i === centerIndex ? "bg-warm-700 dark:bg-warm-300 scale-[1.3]" : "bg-warm-300 dark:bg-warm-700"}`} />
            ))}
          </div>
