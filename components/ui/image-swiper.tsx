    ...
    >
      {cardOrder.map((originalIndex, displayIndex) => (
        <article
          ...
          // Branding: restyled to warm-palette (border-warm-200/800, rounded-2xl,
          // shadow-xl) instead of the snippet's border-slate-400 / rounded-xl /
          // shadow-md so the swiper matches the rest of the page chrome.
          className="image-card absolute cursor-grab active:cursor-grabbing
                     place-self-center border border-warm-200/60 dark:border-warm-800/60
                     rounded-2xl shadow-xl overflow-hidden will-change-transform"
          style={{
        ...
        >
          <img
            ...
            alt={alt?.[originalIndex] ?? `Swiper image ${originalIndex + 1}`}
            className="w-full h-full object-cover select-none pointer-events-none"
            draggable={false}
          />
          {captions && captions[originalIndex] && (
            <figcaption
              className="absolute inset-x-0 bottom-0 z-10 px-4 py-3 bg-gradient-to-t from-black/70 via-black/35 to-transparent pointer-events-none"
